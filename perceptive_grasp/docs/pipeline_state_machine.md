...
主循环每 50 ms 调用一次 `SpinOnce()`。机械臂运动和夹爪动作通过后台 `std::future`
执行，状态机只轮询动作结果：
...
抓取成功后，状态机在 `HOMING` 阶段回到观察姿态并进入 `DONE`。语音模式下没有下一条
抓取命令时，系统保持在观察姿态等待；收到 `结束`、`待命`、`休息`、`回家`、`回 home`、
`回初始`、`end` 或 `home` 后回到 Home 姿态并退出程序。
