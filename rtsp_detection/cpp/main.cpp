 ...
 *
 * RTSP Detection Application
 * USB Camera → MJPEG → VDEC(NV12) → [AI Detection] → VENC(H.264) → RTSP
 * + HTTP control API for enabling/disabling inference
 */
...
static void print_usage(const char* prog) {
    fprintf(stderr,
            ...
            "  --hls-dir <path>      HLS output directory (default: /tmp/hls)\n"
            "  --help                Show this help\n"
            "\n"
            "Example:\n"
...
int main(int argc, char* argv[]) {
    ...
    printf("=== RTSP Detection Application ===\n");
    ...
    // Start HTTP server
    HttpServer http(&pipeline, http_port, web_root, hls_dir);
    http.Start();
